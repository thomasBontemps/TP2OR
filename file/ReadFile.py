...
def readFile(g):
    ...
    while i[0] != ')':  # Links
        ...
        listLinks.append(l)
        g.addEdge(l.getInput(), l.getOutput(), l.getSUCtotal())  # mettre le set-up cost total pour avoir le chemin le plus court
        g.addEdge(l.getInput(), l.getOutput(), l.getSUCtotal(), typeMatrix=False)  # mettre le set-up cost total pour avoir le chemin le plus court
        i = lignes.pop(0)
